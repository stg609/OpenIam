...
1. ```
   docker run -p 5002:443 
    ...
    ``` 
    这个命令使用的是开发环境，并把容器中的 `443` 映射到了本机的 `5002` 端口，同时设置了 Https 的证书（基于 OpenSSL)。当命令成功执行后，会在控制台打印出当前正在监听的端口。
    另外，这个命令并没有配置**钉钉**、**企业微信**相关的配置，而是使用我测试用的钉钉及微信，如无法正常使用，则可以采用环境变量方式进行替换，具体请参考 `appsettings.json`。
